...
interface FormValues {
  noiDungDuyetTongBo: string;
}

const ModelSuaNoiDungTongBo = (props: Props) => {
  ...
  const onSubmit = (values: FormValues) => {
    const params2 = {
      ...
      trangThai: 0,
      noiDungDuyetSoBo: "",
      noiDungDuyetTongBo: values.noiDungDuyetTongBo,
        ...
        <div className="px-6 py-4 border-b border-gray-100">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-gray-800">
              Sửa nội dung duyệt tổng bộ mẫu {data?.tenMau}
            </h3>
            <button
              onClick={handleClose}
        ...
        <div className="p-6 space-y-6">
          <div>
            <label className="text-sm font-medium text-gray-700">
              Nội dung duyệt tổng bộ *
            </label>
            <div className="flex items-center gap-2">
              <textarea
                {...register("noiDungDuyetTongBo")}
                className="w-full border p-2 max-h-32 min-h-32 border-gray-300 rounded"
              />
            </div>
            {errors.noiDungDuyetTongBo && (
              <p className="text-xs text-red-600">
                {errors.noiDungDuyetTongBo.message}
              </p>
            )}
          </div>
...
export default ModelSuaNoiDungTongBo;
