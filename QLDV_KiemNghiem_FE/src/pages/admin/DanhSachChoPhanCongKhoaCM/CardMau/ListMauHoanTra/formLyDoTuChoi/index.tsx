...
const FormLyDoTuChoi = (props: Props) => {
  const { isOpen, onClose, title, message, type, dataSave, typeConform } =
    props;
  ...
  const { mutate } = mutationBLDPheDuyetHoanTraMau({
    queryKey: "mutationBLDPheDuyetHoanTraMau",
    onSuccess: (data: any) => {
      if (data.status === 200) {
        showNotification({
          message: `${
  ...
  const onSubmit = (data: { note: string }) => {
    if (typeConform === typeConfirmation.TuChoi) {
      const params = {
        maMau: dataSave?.maMau,
        maKhoa: dataSave?.maKhoa,
        message: data.note,
        action: false,
      };
