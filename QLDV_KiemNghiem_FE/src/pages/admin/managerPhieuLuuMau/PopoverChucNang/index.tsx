...
const PopoverChucNang = (props: Props) => {
  ...
  return (
    ...
    >
      <button
        onClick={() =>
          navigate(APP_ROUTES.TUNA_ADMIN.QUAN_LY_PHIEU_LUU_MAU.create_mau_luu)
        }
      ...
      >
        ...
        <span className="text-lg/6 font-semibold capitalize">Thêm Mẫu lưu</span>
      </button>
      <button
        onClick={() =>
          navigate(APP_ROUTES.TUNA_ADMIN.QUAN_LY_PHIEU_LUU_MAU.edit_mau_luu)
        }
        className="text-cyan-800 flex gap-2 items-center p-2 bg-white text-center rounded-lg border-[2px] border-cyan-800 cursor-pointer hover:text-orange-400 hover:shadow-[0_4px_4px_rgba(0,0,0,0.25)]"
      >
        <FaUserEdit className="w-6 h-6" />
        <span className="text-lg/6 font-semibold capitalize">Sửa Mẫu lưu</span>
      </button>
    </Popover>
  );
};
